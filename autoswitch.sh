...
if [[ -z "$best_server" ]]; then
  echo "None is available. Aborted." >>/tmp/v2ray-autoswitch.log
  exit
fi

last=$(cat /tmp/v2ray-autoswitch-last.log)
if [[ "$best_server" == "$last" ]]; then
  echo "same as the last best_server. No need to restart service". >>/tmp/v2ray-autoswitch.log
  exit
fi

echo ${best_server} >/tmp/v2ray-autoswitch-last.log
